    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <h1 class="text-3xl font-bold text-white tracking-tight flex items-center gap-3">
          ...
          Pembayaran Baju Toko (Multi-Item)
        </h1>
        <p class="text-slate-400 text-sm mt-1">
          Halaman untuk admin membayar baju yang ada di toko. Berisi jenis barang, harga, jumlah, total harga, dan catatan.
        </p>
      </div>

      <button 
        @click="showForm = !showForm"
        class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-semibold shadow-lg shadow-blue-500/20 transition"
      >
    ...
    <transition name="slide">
      <div v-if="showForm" class="glass-card rounded-2xl p-6 border border-slate-700/60 shadow-xl space-y-6">
        <h2 class="text-lg font-bold text-white flex items-center gap-2 border-b border-slate-800 pb-3">
          <ReceiptIcon class="w-5 h-5 text-blue-400" />
          Form Pembayaran Baju Toko
        </h2>

        <form @submit.prevent="submitPembayaranToko" class="space-y-6">
          ...
          <div class="bg-slate-900/80 p-5 rounded-2xl border border-slate-800 space-y-4">
            <div class="flex items-center justify-between border-b border-slate-800 pb-3">
              <div class="flex items-center gap-2">
                <PackageIcon class="w-5 h-5 text-blue-400" />
                <h3 class="font-bold text-white text-base">Daftar Pilihan Jenis Barang Baju</h3>
              </div>
              ...
              <button
                type="button"
                @click="addItemRow"
                class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-500/20 hover:bg-blue-500/30 text-blue-300 border border-blue-500/40 text-xs font-bold transition"
              >
            ...
            <div v-if="form.items.length === 0" class="text-center py-6 text-slate-500 text-xs font-medium">
              Belum ada jenis barang yang dipilih. Klik <span class="text-blue-400 font-semibold">+ Tambah Jenis Barang</span> di atas.
            </div>
            ...
            <div v-else class="space-y-3">
              ...
              >
                ...
                <div class="md:col-span-2">
                  <label class="block text-[11px] font-semibold text-slate-400 uppercase md:hidden mb-1">Jumlah</label>
                  <input
                    type="number"
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4 pt-3 border-t border-slate-800">
              ...
              <div class="flex items-center gap-3">
                ...
                <span class="text-xl font-bold text-blue-400 font-mono bg-blue-500/10 px-4 py-1.5 rounded-xl border border-blue-500/30">
          ...
          <div class="flex justify-end gap-3 border-t border-slate-800 pt-4">
            ...
            <button
              ...
              class="px-6 py-2.5 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-semibold text-sm shadow-lg shadow-blue-500/20 flex items-center gap-2"
            >
              <Loader2Icon v-if="loadingSubmit" class="w-4 h-4 animate-spin" />
              <span>Simpan Pembayaran Toko</span>
            </button>
          </div>
        </form>
      </div>
          ...
          <tbody class="divide-y divide-slate-800/60">
            <tr v-if="loadingTable" class="text-center">
              <td colspan="6" class="py-12 text-slate-400">
                <Loader2Icon class="w-6 h-6 animate-spin mx-auto text-blue-400 mb-2" />
            ...
            >
              <!-- Kode Pembelian -->
              <td class="py-3.5 px-4 font-mono text-xs font-semibold text-blue-400">
                {{ item.kode_pembelian }}
              </td>
              ...
              <td class="py-3.5 px-4">
                <div v-if="item.items && item.items.length > 0" class="space-y-1">
                  ...
                  >
                    ...
                    <span class="font-mono text-blue-400 font-semibold text-[11px]">Rp {{ formatNumber(line.harga) }} × {{ line.jumlah }} pcs</span>
                  </div>
                </div>
