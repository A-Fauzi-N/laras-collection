...
import { penjualan, penjualanItem, jenisBarang, pembayaranPenjualan } from '../db/schema.js';
  ...
  .post(
    ...
    async ({ body, set }) => {
      try {
        ...
        const initialDP = Number(body.dp_awal || 0);
        ...
        const tiktokHandle = body.nama_tiktok.startsWith('@') ? body.nama_tiktok : `@${body.nama_tiktok}`;
